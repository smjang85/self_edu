...
public class Test03 {
    public static int solution(int m, int n, int[][] puddles) {
        ...
        for (int i = 0; i < n; i++) { // 시작점은 1로 저장   3
          for (int j = 0; j < m; j++) {  //4
            ...
            if(i != 0)
              street[i][j] = street[i][j] + street[i - 1][j] % 1000000007; // 숫자가 이 값을 초과할 수 있기 때문에 계산 과정에서 나머지 구하기

            if(j != 0)
              street[i][j] = street[i][j] + street[i][j - 1] % 1000000007; // 왼쪽
          }
        }
